...
export namespace Provider {
  ...
  const BUNDLED_PROVIDERS: Record<string, (options: any) => SDK> = {
    ...
    // Free models provider - uses OpenRouter
    "free-models": createOpenRouter,
  }
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from codepwn.json provider config
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "codepwn",
          },
        },
      }
    },
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("codepwn")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if codepwn provider is available before using it
    const codepwnProvider = await state().then((state) => state.providers["codepwn"])
    if (codepwnProvider && codepwnProvider.models["gpt-5-nano"]) {
      return getModel("codepwn", "gpt-5-nano")
    }
