...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall codepwn and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CodePwn")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g codepwn-ai",
      pnpm: "pnpm uninstall -g codepwn-ai",
      bun: "bun remove -g codepwn-ai",
      yarn: "yarn global remove codepwn-ai",
      brew: "brew uninstall codepwn",
      choco: "choco uninstall codepwn",
      scoop: "scoop uninstall codepwn",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall codepwn -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using CodePwn!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# codepwn") || content.includes(".codepwn/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".codepwn/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".codepwn/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".codepwn"))
    ) {
      continue
    }
