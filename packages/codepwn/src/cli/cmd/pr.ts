...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run codepwn",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for codepwn session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found codepwn session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`codepwn import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting codepwn...")
        UI.println()

        // Launch codepwn TUI with session ID if available
        const { spawn } = await import("child_process")
        const codepwnArgs = sessionId ? ["-s", sessionId] : []
        const codepwnProcess = spawn("codepwn", codepwnArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          codepwnProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`codepwn exited with code ${code}`))
          })
          codepwnProcess.on("error", reject)
        })
      },
    })
  },
