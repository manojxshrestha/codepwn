...
import type { Event, OpencodeClient, SessionMessageResponse } from "@codepwn-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `codepwn auth login` in the terminal",
        name: "Login with codepwn",
        id: "codepwn-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // CodePwn stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const codepwnProvider = providers.find((p) => p.id === "codepwn")
    if (codepwnProvider) {
      if (codepwnProvider.models["big-pickle"]) {
        return { providerID: "codepwn", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(codepwnProvider.models))
      if (best) {
        return {
